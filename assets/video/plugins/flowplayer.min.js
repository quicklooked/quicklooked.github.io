(function(global, factory) {
    ...
    */
    ...
    var TAGS = INLINE.concat([
        "div", "strong", "video", "img" // block elements
        , "ol", "ul", "li", "script" // invisible elements
    ]);
    /**
    ...
    function el(name, opts, children) {
        ...
        tag.append(children.filter(function(child) {
            return child
        }));
    ...
    function methods(el) {
        ...
        el.addClass = function(names) {
            names.split(SPACE).forEach(function(name) {
                if (!el.hasClass(name)) el.className += (el.className ? SPACE : EMPTY) + name;
            });
            return el
        };
        // removeClass
        el.removeClass = function(names) {
            ...
            el.className = current_classes.filter(function(klass) {
    ...
    function mq(query, root) {
        if (typeof query == "string" && query[0] == "<") return el(query.slice(1, -1), root)
        if (typeof query !== "string") return createMQElement(query)
        var node = (root instanceof HTMLElement && root.querySelector(".fp-" + query)) || document.querySelector(query);
    ...
    function support() {
        var in_browser = typeof document !== "undefined" && typeof window !== "undefined";
        var UA = in_browser ? navigator.userAgent : "",
            IS_IPHONE = /iP(hone|od)/i.test(UA) && !/iPad/.test(UA) && !/IEMobile/i.test(UA),
            IS_ANDROID = /Android/.test(UA) && !/Firefox/.test(UA),
            IS_IE_MOBILE = /IEMobile/.test(UA),
            WIN_PHONE_VER = IS_IE_MOBILE ? parse_ver(UA, /Windows Phone (\d+\.\d+)/) : 0,
        ...
        var self = {
            controls: !IS_IPHONE && (!IS_IE_MOBILE || (WIN_PHONE_VER >= 8.1 && IE_MOBILE_VER >= 11)),
            video: function(type) {
                return in_browser && document.createElement("video").canPlayType(type)
            },
            lang: in_browser && (window.navigator.userLanguage || window.navigator.language),
            android: IS_ANDROID,
            iphone: IS_IPHONE,
            ie: IE_VERSION,
            ios: in_browser && (/iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream)
        };
        return self
    }
    mq.window && mq.window.on([Events.LOAD, Events.RESIZE], function() {
    ...
    function classify(opts, root, video) {
        ...
        video.on(Events.ENDED, function(e) {
            setTimeout(function() {
                ...
                // issues/179
                tween(root, states.ENDED, states.PLAYING,
        ...
        // progress
        video.on([Events.WAITING, Events.CAN_PLAY, Events.LOAD_START], function(e) {
            ...
            if (e.type == Events.LOAD_START && opts.preload == "none" && !opts.autoplay) return
            ...
            root.toggleClass(states.WAITING,
    ...
    function aspect_ratio_plugin(opts, root, video) {
        ...
        aspect_ratio.set = function(ratio) {
            ...
            aspect_ratio.css("padding-top", !isNaN(ratio) ?
                (ratio * 100) + "%" :
                (ratio));
        };
        ...
        video.on(Events.METADATA, function() {
            aspect_ratio.set(opts.ratio ||
                video.videoHeight && video.videoWidth && (video.videoHeight / video.videoWidth));
        });
    ...
    function in_viewport(root) {
        ...
        return (rect.top <= offset && rect.top + player_height > view_height - offset ||
    ...
    mq.window && mq.window.on(Events.SCROLL, function _onscroll() {
        instances.forEach(update_visible);
    });
    function viewport(opts, _root, video) {
        update_visible(video)
            .on(Events.VIEW_LEAVE, video.pause);
    ...
    function absorb_promised_play(promise, video, opts) {
        ...
        return promise.catch(function(err) {
                ...
                if (err.name == "AbortError" && err.code == 20) return no_autoplay(video)
    ...
    function parse(src) {
        ...
        return types.slice(0, -1).split(",").map(function(type) {
            return {
                src: els[0] + type,
    ...
    function api(opts, root, video) {
        ...
        video.setSrc = function(src) {
            ...
            src.forEach(function(data, i) {
                ...
                video.append(mq("<source>", data)).on(Events.ERROR, function source_failed(e) {
                    e.target.remove();
                    ...
                    video.emit(Events.ERROR, {
                        message: code == 4 ? "Video not found: " + path : (data.type || path) + " is not supported",
                        videoType: data.type,
                        code: code,
                        src: path
                    });
        ...
        video.toggleMute = function(flag) {
            ...
            if (!video.muted && !video.volume) video.volume = .5;
    ...
    function Player(conf, root) {
        ...
        if (player.async && !player.paused) {
            root.addClass(states.PLAYING)
                .addClass(states.SEAMLESS);
            return player
        }
    ...
    function available(cb) {
        if (typeof document == "undefined" || ~READY.indexOf(document.readyState)) return cb()
    ...
    function modules() {
        ...
        var mod = self.flowplayer ? self.flowplayer : {
            extensions: []
        };
    ...
    Token.validate = function(token) {
        ...
        if (!token || !token.raw) {
            if ("localhost" === window.location.hostname || "production" === "dev") return true
        ...
        if (token.data.domain &&
            token.data.domain.length &&
            !token.data.domain.some(c)) throw new Error("Domain not allowed")
    ...
    function flowplayer$1(arg, opts) {
        ...
        var root$$1 = mq(arg);
        /*eslint no-console: 0*/
        if (!root$$1) return console.error(new Error("Flowplayer: '" + arg.toString() + "' not found"))
        // noop on double-invoke
        if (root$$1.video) return root$$1.video
        var v = check_token$$1(opts.token);
        if (!v.success) return fatal_error(root$$1, null, {
            reason: "token",
            details: v.message
        })
        ...
        if (!support().video) return root$$1.append(mq.img({
            src: opts.poster
        }))
        var video = reaper.put("video",
            Player(opts, root$$1));
    ...
    function fatal_error(root$$1, video, data) {
        ...
        root$$1.append(mq.a({
            ...
            target: "_blank",
            href: "https://flowplayer.com/help/developers/troubleshooting?reason=" + data.reason
        }, "Troubleshoot"));
        root$$1.append(mq.a({}, mq.img({
            src: "https://flowplayer.com/user/pages/images/logo-white.png"
        })));
        console.error(new Error(data.details || data.reason));
    }
    var PING = ["siteGroupId", "siteId", "id", "created", "sessionId", "position"];
    var REQUEST = ["siteGroupId", "siteId", "id", "playerId", "playerVersion", "sessionId", "deviceInfo", "created", "source", "category"];
    /**
        ...
        return function(data) {
            try {
                ...
                var xhr = new XMLHttpRequest(),
                    ...
                    endpoint = ["https://ptm.flowplayer.com", type, data.siteGroupId, action].join("/");
                xhr.timeout = 2500;
    ...
    function find_segments(duration) {
        ...
        // $FlowFixMe
        return [segment_duration, Array.apply(null, Array(NUMBER_OF_SEGMENTS)).map(function(_, i) {
    ...
    function ping_vod(video, state, endpoint) {
        ...
        endpoint.ping(
            payload_whitelist(PING,
                assign({}, state.payload, seen)));
    }
    ...
    function track_vod(video, state, endpoint) {
        ...
        video.on(Events.SEEKED, function() {
    ...
    var setup = function(config, state, video) {
        ...
        endpoint.display(
            payload_whitelist(REQUEST, state.payload));
    ...
    function analytics(config, _root, video) {
        if (!ACL.from_token(config.token).is_granted(ACL.permissions.ANALYTICS)) return
        var state = {};
        state.payload = {
            playerVersion: "1.2.1",
            ...
            source: encodeURI(document.location.href.replace(document.location.protocol + "//", ""))
        };
        video.on(Events.CONFIG, function(e) {
    ...
    function orient(type) {
        window.matchMedia("(orientation: " + type + ")").addListener(function(query) {
            instances.length && instances.forEach(function(instance) {
     ...
     */
    if (typeof window !== "undefined" && window.matchMedia)[LANDSCAPE, PORTRAIT].forEach(orient);
    /**
    ...
    function ontouch(_opts, root, video) {
        if (typeof window !== "undefined" && "ontouchstart" in window) root.addClass(IS_TOUCH_DEVICE);
    ...
    */
    ...
    function volume(opts, controls, video) {
        ...
        var $bar = create_volume_bar(),
            ...
            mute_button = mq.div("fp-volumebtn",
                view_box(VOLUME$1));
        controls.append(mute_button);
        if (AUDIO.volume == .1) controls.append($bar);
        if (opts.muted_initially) storage.setItem(MUTE, opts.muted_initially == true);
        ...
        if (opts.muted_initially) {
            ...
            video.one(Events.PAUSE, function() {
                video.volume = 1 * (storage.getItem(VOLUME) || .5);
                video.play();
            });
            return $volume
        }
    ...
    !(function() {
        ...
        $doc.on("webkitfullscreenchange mozfullscreenchange MSFullscreenChange msfullscreenchange", function() {
            $doc.emit(Events.FULLSCREEN_CHANGE,
        ...
        if (!proto.requestFullScreen) {
            ["webkit", "moz", "ms"].forEach(function(prefix) {
                // request
                var req = proto[prefix + "RequestFullScreen"] || proto["msRequestFullscreen"]
                    // exit (W3C Proposal)
                    ,
    ...
    function middle(opts, ui, video) {
        ...
        play($middle, video);
        waiting_ele($middle);
        opts.mouseover_play && video.root.on([Events.MOUSE_ENTER, Events.MOUSE_LEAVE], function(e) {
            video.togglePlay(e.type == "mouseenter");
        });
        ICON_SETS.forEach(function(i) {
            $middle.append(
                mq.div("fp-switch",
                    [icons["PLAY_" + i], icons["PAUSE_" + i]].map(view_box)).addClass("fp-play-" + i));
        });
        ui.append($middle);
        return $middle
    }
    var tick = typeof requestAnimationFrame === "function" ?
    ...
    function link(reaper, name, rest, type) {
        ...
        rest = (typeof rest == "string") ?
            " " + rest :
            "";
        type = type || div;
        var base_name = fp(name);
        return reaper.put(name,
            type(base_name + rest))
    }
    function fp(rest) {
        return "fp-" + rest
    ...
    function init_controls(container) {
        ...
        timeline.append(
            [buffer, progress, hover, timestamp]);
        controls.append(
            [btns, live, elapsed, timeline, duration]);
        Reaper(controls);
        btns.append(
            [view_box(SMALL_PAUSE), view_box(SMALL_PLAY)]);
        progress.append(dragger);
        container.append(controls);
        return controls
    }
    function tooltip_offset(tooltip, percent) {
        var offset = ((tooltip.innerWidth() / tooltip.parentElement.innerWidth()) * 100) / 2;
    ...
    function loadedmetadata(e, video, controls) {
        video.duration && controls.duration.txt(
            prettyTime(video.duration));
    }
    function timeupdate(e, video, controls) {
        controls.elapsed.txt(
            prettyTime(video.currentTime));
    }
    function timeline_repaint(e, video, controls) {
        controls.progress.css("width",
    ...
    function tooltip_update(e, video, controls) {
        ...
        tooltip.txt(
            prettyTime((percent / 100) * video.duration));
    }
    ...
    function is_dvr(video, dvr_threshold) {
        ...
        try {
            return (buffered.end(last_epoc) - buffered.start(last_epoc)) > dvr_threshold
        } catch (err) {
            console.error(err);
    ...
    function seconds_in_past(video) {
        return video.currentTime - (seconds_from_head(video, video.dvr_offset || 0) + REBUFFER_BUDGET)
    }
    ...
    function timeupdate$1(_, video, controls) {
        ...
        var timestamp = video.dvr_offset > 0 ?
    ...
    function Behavior(args) {
        ...
        this.route = function(kind, e) {
            ...
            throw new Error("could not handle routed event of type (" + kind + ":" + e.type + ")")
        };
    }
    ...
    function controls(opts, root, video) {
        ...
        root.reaper.put("timeline",
            controls.timeline);
        root.reaper.put("controls",
            controls);
        ...
        function is_live(video) {
            return typeof video.opts.live === "undefined" ? video.duration === Infinity : video.opts.live
        }
        // progressbar
    ...
    function printError(ele, msg, details) {
        ...
        if (details) {
            err.append(
                mq.div("fp-error-details").txt(details));
        }
    }
    ...
    function title_and_desc(config, root, video) {
        var container = root.append(mq.div({
            class: "fp-footer"
        }));
        video.on(Events.CONFIG, function(e) {
            var data = e.data,
    ...
    function ui(config, root, video) {
        ...
        ui.on(Events.MOUSE_MOVE, function() {
            ...
            move_timeout = (video.paused == false) && setTimeout(function() {
                root.removeClass("is-hovered");
            }, 2000);
        });
    }
    ...
    function metering(config, root, video) {
        if (ACL.from_token(config.token).is_granted(ACL.permissions.NO_METERING)) return
        var token_id = Token.id(config.token);
        if (!token_id) return
        var meter_response = function(xhr) {
            if (xhr.status === 403) video.emit(Events.NON_RECOVERABLE_ERROR, {
                reason: "token",
    ...
    function meter(token_id, cb) {
        ...
        var xhr = new XMLHttpRequest(),
            body = JSON.stringify(data);
        xhr.timeout = 2500;
        xhr.open("POST", flowplayer.METERING_URL || "https://pmi.flowplayer.com/in", true);
        xhr.send(body);
