...
var vp_unmute_button_title = document.querySelector(".vp-unmute-button-title");

function left_backward() {
  var vid_currentTime = video.currentTime;
  if(vid_currentTime = 0) {
    video.currentTime == 0;
    vp_unmute_button_title.innerHTML = (`${neatTime(video.currentTime)}` + " / " + `${neatTime(video.duration)}`);
  }
  else {
    video.currentTime == vid_currentTime - 5;
    vp_unmute_button_title.innerHTML = (`${neatTime(video.currentTime)}` + " / " + `${neatTime(video.duration)}`);
...
function right_backward() {
  var vid_currentTime = video.currentTime;
  if(vid_currentTime = video.duration) {
...
vp_left_zone.addEventListener("click", left_backward, false);
vp_right_zone.addEventListener("click", right_backward, false);
