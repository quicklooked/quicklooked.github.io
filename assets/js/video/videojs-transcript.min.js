(function(window, videojs) {
  ...
  var widget = function(plugin) {
    ...
    var createLine = function(cue) {
      var line = utils.createEl('p', '-line');
      var timestamp = utils.createEl('a', '-timestamp sentence');
      var text = utils.createEl('a', '-text sentence');
      text.setAttribute('data-begin', cue.startTime);
      text.setAttribute('tabindex', my._options.tabIndex || 0);
    ...
    var setCue = function(time) {
      var active, i, text, begin, end;
      var texts = my.body.children;
      for(i = 0; i < texts.length; i++) {
        line = texts[i];
        begin = line.getAttribute('data-begin');
        if(i < texts.length - 1) {
          end = texts[i + 1].getAttribute('data-begin');
        } else {
          end = plugin.player.duration() || Infinity;
        }
        if(time > begin && time < end) {
          if(!text.classList.contains('is-active')) { // don't update if it hasn't changed
